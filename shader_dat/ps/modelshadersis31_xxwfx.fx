sampler A_Occ_sampler;
sampler Color_1_sampler;
float4 CubeParam;
sampler Shadow_Tex_sampler;
float4 ambient_rate;
float4 ambient_rate_rate;
sampler cubemap_mask_sampler;
samplerCUBE cubemap_sampler;
float3 fog;
float4 g_All_Offset;
float g_ShadowUse;
float4 g_TargetUvParam;
float4 light_Color;
float4 lightpos;
float4 muzzle_light;
float4 muzzle_lightpos;
sampler normalmap_sampler;
float4 point_light1;
float4 point_lightpos1;
float4 prefogcolor_enhance;
float4 specularParam;
float4 tile;
float4x4 viewInverseMatrix;

struct PS_IN
{
	float3 color : COLOR;
	float4 texcoord : TEXCOORD;
	float3 texcoord1 : TEXCOORD1;
	float4 texcoord2 : TEXCOORD2;
	float3 texcoord3 : TEXCOORD3;
	float3 texcoord4 : TEXCOORD4;
	float4 texcoord7 : TEXCOORD7;
	float4 texcoord8 : TEXCOORD8;
};

float4 main(PS_IN i) : COLOR
{
	float4 o;

	float4 r0;
	float4 r1;
	float4 r2;
	float4 r3;
	float3 r4;
	float4 r5;
	float4 r6;
	float3 r7;
	float4 r8;
	float3 r9;
	r0 = g_All_Offset.xyxy + i.texcoord.zwxy;
	r1 = tex2D(Color_1_sampler, r0.zwzw);
	r2 = r1.w + -0.8;
	clip(r2);
	r2 = tex2D(A_Occ_sampler, r0);
	r0 = tex2D(cubemap_mask_sampler, r0.zwzw);
	r3.xy = -r2.yy + r2.xz;
	r0.w = max(abs(r3.x), abs(r3.y));
	r0.w = r0.w + -0.015625;
	r1.w = (-r0.w >= 0) ? 0 : 1;
	r0.w = (r0.w >= 0) ? -0 : -1;
	r0.w = r0.w + r1.w;
	r0.w = (r0.w >= 0) ? -r0.w : -0;
	r2.xz = (r0.ww >= 0) ? r2.yy : r2.xz;
	r3.xy = -r1.yy + r1.xz;
	r0.w = max(abs(r3.x), abs(r3.y));
	r0.w = r0.w + -0.015625;
	r1.w = (-r0.w >= 0) ? 0 : 1;
	r0.w = (r0.w >= 0) ? -0 : -1;
	r0.w = r0.w + r1.w;
	r0.w = (r0.w >= 0) ? -r0.w : -0;
	r1.xz = (r0.ww >= 0) ? r1.yy : r1.xz;
	r3.xyz = r1.xyz * i.color.xyz;
	r1.xyz = r1.xyz * i.color.xyz + specularParam.www;
	r4.xyz = r2.xyz * r3.xyz;
	r4.xyz = r4.xyz * ambient_rate.xyz;
	r4.xyz = r4.xyz * ambient_rate_rate.xyz;
	r5.xyz = i.texcoord3.xyz;
	r6.xyz = r5.yzx * i.texcoord2.zxy;
	r5.xyz = i.texcoord2.yzx * r5.zxy + -r6.xyz;
	r6.xy = g_All_Offset.xy;
	r6.xy = i.texcoord.xy * tile.xy + r6.xy;
	r6 = tex2D(normalmap_sampler, r6);
	r6.xyz = r6.xyz + -0.5;
	r5.xyz = r5.xyz * -r6.yyy;
	r0.w = r6.x * i.texcoord2.w;
	r5.xyz = r0.www * i.texcoord2.xyz + r5.xyz;
	r5.xyz = r6.zzz * i.texcoord3.xyz + r5.xyz;
	r6.xyz = normalize(r5.xyz);
	r0.w = dot(lightpos.xyz, r6.xyz);
	r1.w = r0.w;
	r2.w = r1.w + -0.5;
	r3.w = max(r2.w, 0);
	r2.xyz = r2.xyz + r3.www;
	r5.xyz = r2.xyz * r3.xyz;
	r7.xyz = point_lightpos1.xyz + -i.texcoord1.xyz;
	r8.xyz = normalize(r7.xyz);
	r2.w = dot(r8.xyz, r6.xyz);
	r7.xyz = r2.www * point_light1.xyz;
	r7.xyz = r7.xyz * i.texcoord8.xxx;
	r8.xyz = muzzle_lightpos.xyz + -i.texcoord1.xyz;
	r9.xyz = normalize(r8.xyz);
	r2.w = dot(r9.xyz, r6.xyz);
	r8.xyz = r2.www * muzzle_light.xyz;
	r7.xyz = r8.xyz * i.texcoord8.zzz + r7.xyz;
	r2.w = 1 / i.texcoord7.w;
	r8.xy = r2.ww * i.texcoord7.xy;
	r8.xy = r8.xy * float2(0.5, -0.5) + 0.5;
	r8.xy = r8.xy + g_TargetUvParam.xy;
	r8 = tex2D(Shadow_Tex_sampler, r8);
	r2.w = r8.z + g_ShadowUse.x;
	r8.xyz = r1.www * light_Color.xyz;
	r7.xyz = r8.xyz * r2.www + r7.xyz;
	r5.xyz = r5.xyz * r7.xyz;
	r3.w = dot(lightpos.xyz, i.texcoord3.xyz);
	r0.w = r0.w + -r3.w;
	r0.w = r0.w + 1;
	r4.xyz = r4.xyz * r0.www + r5.xyz;
	r5.x = dot(r6.xyz, transpose(viewInverseMatrix)[0].xyz);
	r5.y = dot(r6.xyz, transpose(viewInverseMatrix)[1].xyz);
	r5.z = dot(r6.xyz, transpose(viewInverseMatrix)[2].xyz);
	r0.w = dot(i.texcoord4.xyz, r5.xyz);
	r0.w = r0.w + r0.w;
	r5.xyz = r5.xyz * -r0.www + i.texcoord4.xyz;
	r5.w = -r5.z;
	r5 = tex2D(cubemap_sampler, r5.xyww);
	r5 = r5 * ambient_rate_rate.w;
	r5.xyz = r0.xyz * r5.xyz;
	r0.w = r5.w * CubeParam.y + CubeParam.x;
	r5.xyz = r0.www * r5.xyz;
	r5.xyz = r2.xyz * r5.xyz;
	r3.xyz = r3.xyz * r5.xyz;
	r7.xyz = r3.xyz * CubeParam.zzz + r4.xyz;
	r3.xyz = r3.xyz * CubeParam.zzz;
	r3.xyz = r4.xyz * -r3.xyz + r7.xyz;
	r0.w = dot(-i.texcoord1.xyz, -i.texcoord1.xyz);
	r0.w = 1 / sqrt(r0.w);
	r4.xyz = -i.texcoord1.xyz * r0.www + lightpos.xyz;
	r7.xyz = normalize(r4.xyz);
	r0.w = dot(r7.xyz, r6.xyz);
	r3.w = pow(r0.w, specularParam.z);
	r4.x = (-r1.w >= 0) ? 0 : 1;
	r1.w = r1.w * r4.x;
	r0.w = (-r0.w >= 0) ? 0 : r4.x;
	r0.w = r3.w * r0.w;
	r4.xyz = r1.www * light_Color.xyz;
	r4.xyz = r0.www * r4.xyz;
	r0.xyz = r0.xyz * r4.xyz;
	r0.xyz = r2.www * r0.xyz;
	r0.xyz = r2.xyz * r0.xyz;
	r0.w = abs(specularParam.x);
	r0.xyz = r0.www * r0.xyz;
	r0.xyz = r0.xyz * r1.xyz + r3.xyz;
	r1.y = 1;
	r0.w = r1.y + -CubeParam.z;
	r0.xyz = r5.xyz * r0.www + r0.xyz;
	r1.xyz = fog.xyz;
	r0.xyz = r0.xyz * prefogcolor_enhance.xyz + -r1.xyz;
	o.xyz = i.texcoord8.www * r0.xyz + fog.xyz;
	o.w = prefogcolor_enhance.w;

	return o;
}

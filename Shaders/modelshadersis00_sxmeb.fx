sampler A_Occ_sampler;
sampler Color_1_sampler;
sampler Color_2_sampler;
sampler Shadow_Tex_sampler;
float4 ambient_rate;
float4 ambient_rate_rate;
float3 fog;
float4 g_All_Offset;
float g_ShadowUse;
float4 g_TargetUvParam;
float4 light_Color;
float4 lightpos;
float4 muzzle_light;
float4 muzzle_lightpos;
float4 point_light1;
float4 point_lightpos1;
float4 prefogcolor_enhance;
float4 specularParam;
float4 tile;

struct PS_IN
{
	float4 color : COLOR;
	float4 texcoord : TEXCOORD;
	float3 texcoord1 : TEXCOORD1;
	float3 texcoord3 : TEXCOORD3;
	float4 texcoord7 : TEXCOORD7;
	float4 texcoord8 : TEXCOORD8;
};

float4 main(PS_IN i) : COLOR
{
	float4 o;

	float4 r0;
	float4 r1;
	float4 r2;
	float3 r3;
	float4 r4;
	float3 r5;
	float3 r6;
	r0.xy = g_All_Offset.xy;
	r0.xy = i.texcoord.xy * tile.zw + r0.xy;
	r0 = tex2D(Color_2_sampler, r0);
	r1.xy = -r0.yy + r0.xz;
	r2.x = max(abs(r1.x), abs(r1.y));
	r1.x = r2.x + -0.015625;
	r1.y = (-r1.x >= 0) ? 0 : 1;
	r1.x = (r1.x >= 0) ? -0 : -1;
	r1.x = r1.x + r1.y;
	r1.x = (r1.x >= 0) ? -r1.x : -0;
	r0.xz = (r1.xx >= 0) ? r0.yy : r0.xz;
	r1 = g_All_Offset.xyxy + i.texcoord.zwxy;
	r2 = tex2D(Color_1_sampler, r1.zwzw);
	r1 = tex2D(A_Occ_sampler, r1);
	r3.xy = -r2.yy + r2.xz;
	r1.w = max(abs(r3.x), abs(r3.y));
	r1.w = r1.w + -0.015625;
	r3.x = (-r1.w >= 0) ? 0 : 1;
	r1.w = (r1.w >= 0) ? -0 : -1;
	r1.w = r1.w + r3.x;
	r1.w = (r1.w >= 0) ? -r1.w : -0;
	r2.xz = (r1.ww >= 0) ? r2.yy : r2.xz;
	r0.w = r0.w * i.color.w;
	r3.xyz = lerp(r0.xyz, r2.xyz, r0.www);
	r0.x = 1 / ambient_rate.w;
	r0.yzw = r3.xyz * r0.xxx + specularParam.www;
	r2.xyz = r0.xxx * r3.xyz;
	r0.x = dot(-i.texcoord1.xyz, -i.texcoord1.xyz);
	r0.x = 1 / sqrt(r0.x);
	r3.xyz = -i.texcoord1.xyz * r0.xxx + lightpos.xyz;
	r4.xyz = normalize(r3.xyz);
	r0.x = dot(r4.xyz, i.texcoord3.xyz);
	r1.w = pow(r0.x, specularParam.z);
	r3.x = dot(lightpos.xyz, i.texcoord3.xyz);
	r3.y = (-r3.x >= 0) ? 0 : 1;
	r3.x = r3.x * r3.y;
	r0.x = (-r0.x >= 0) ? 0 : r3.y;
	r0.x = r1.w * r0.x;
	r3.xyz = r3.xxx * light_Color.xyz;
	r3.xyz = r0.xxx * r3.xyz;
	r3.xyz = r2.www * r3.xyz;
	r0.x = 1 / i.texcoord7.w;
	r4.xy = r0.xx * i.texcoord7.xy;
	r4.xy = r4.xy * float2(0.5, -0.5) + 0.5;
	r4.xy = r4.xy + g_TargetUvParam.xy;
	r4 = tex2D(Shadow_Tex_sampler, r4);
	r0.x = r4.z + g_ShadowUse.x;
	r3.xyz = r0.xxx * r3.xyz;
	r4.xyz = ambient_rate.xyz;
	r4.xyz = r0.xxx * ambient_rate_rate.xyz + r4.xyz;
	r5.xy = -r1.yy + r1.xz;
	r0.x = max(abs(r5.x), abs(r5.y));
	r0.x = r0.x + -0.015625;
	r1.w = (-r0.x >= 0) ? 0 : 1;
	r0.x = (r0.x >= 0) ? -0 : -1;
	r0.x = r0.x + r1.w;
	r0.x = (r0.x >= 0) ? -r0.x : -0;
	r1.xz = (r0.xx >= 0) ? r1.yy : r1.xz;
	r3.xyz = r1.xyz * r3.xyz;
	r0.x = abs(specularParam.x);
	r3.xyz = r0.xxx * r3.xyz;
	r0.xyz = r0.yzw * r3.xyz;
	r3.xyz = point_lightpos1.xyz + -i.texcoord1.xyz;
	r5.xyz = normalize(r3.xyz);
	r0.w = dot(r5.xyz, i.texcoord3.xyz);
	r3.xyz = r0.www * point_light1.xyz;
	r3.xyz = r3.xyz * i.texcoord8.xxx;
	r5.xyz = muzzle_lightpos.xyz + -i.texcoord1.xyz;
	r6.xyz = normalize(r5.xyz);
	r0.w = dot(r6.xyz, i.texcoord3.xyz);
	r5.xyz = r0.www * muzzle_light.xyz;
	r3.xyz = r5.xyz * i.texcoord8.zzz + r3.xyz;
	r1.xyz = r1.xyz * r4.xyz + r3.xyz;
	r0.xyz = r2.xyz * r1.xyz + r0.xyz;
	r1.xyz = fog.xyz;
	r0.xyz = r0.xyz * prefogcolor_enhance.xyz + -r1.xyz;
	o.xyz = i.texcoord8.www * r0.xyz + fog.xyz;
	o.w = prefogcolor_enhance.w;

	return o;
}
